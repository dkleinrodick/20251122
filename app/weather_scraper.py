...
from app.models import Airport, WeatherData
...
class WeatherScraper:
    ...
    async def update_forecasts(self):
        ...
        logger.info(f"Fetching weather for {len(airports_data)} airports...")
            
        # Reduce batch size to avoid 429s
        chunk_size = 10 
        for i in range(0, len(airports_data), chunk_size):
            ...
            logger.info(f"Processed batch {i+1}-{min(i+chunk_size, len(airports_data))}")
