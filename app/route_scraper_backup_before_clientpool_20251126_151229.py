...
class RouteScraper:
    ...
    async def validate_routes(self, stop_check=None):
        ...
        async with SessionLocal() as session:
            ...
            max_concurrent = max(1, min(val, 200)) # Increased cap for faster validation
            ...
            async def check_route(route_dict):
                nonlocal completed, valid_count, invalid_count
                route_id = route_dict["id"]
                origin = route_dict["origin"]
                dest = route_dict["destination"]

                # Single attempt - retry only on network errors
                ...
                async with sem:
                    ...
                    try:
                        ...
                        # Only check ONE date - if HTTP 200, route is valid regardless of flights
                        check_date = (today_utc + timedelta(days=7)).strftime("%Y-%m-%d")

                        try:
                            await client.search(origin, dest, check_date)
                            # HTTP 200 = Route exists (valid), whether or not there are flights
                            logger.info(f"[VALID] {origin}-{dest}")
                            result = {"id": route_id, "status": "valid"}

                        except httpx.HTTPStatusError as http_err:
                            if http_err.response.status_code == 400:
                                # Check if market doesn't exist
                                try:
                                    err_data = http_err.response.json()
                                    msg = err_data.get("message", "").lower()
                                    if "does not exist" in msg:
                                        # Market doesn't exist = invalid route
                                        logger.info(f"[INVALID] {origin}-{dest} - Market does not exist")
                                        result = {"id": route_id, "status": "invalid"}
                                except:
                                    pass

                            # Any other HTTP error = retry (could be temporary)
                            if result["status"] == "skip":
                                logger.warning(f"[RETRY] {origin}-{dest} - HTTP {http_err.response.status_code}")

                        except Exception as e:
                            # Network/connection error = retry
                            logger.warning(f"[RETRY] {origin}-{dest} - {type(e).__name__}: {e}")

                    except Exception as e:
                        # Auth or setup error = retry
                        logger.warning(f"[RETRY] {origin}-{dest} - Setup error: {e}")
                ...
                async with progress_lock:
                    ...
                    # Update progress every 10 routes or on last route
                    if completed % 10 == 0 or completed == total:
                        pct = int((completed / total) * 100)
                        update_job(self.job_id, progress=pct,
                                 message=f"Validated {completed}/{total} (✓{valid_count} ✗{invalid_count})")

                return result

            # Process all routes at once - no batching, workers continuously process
            logger.info(f"Processing all {total} routes concurrently...")
            update_job(self.job_id, progress=0, message=f"Validating 0/{total}...")

            try:
                # Launch all tasks at once, semaphore controls concurrency
                tasks = [check_route(r) for r in routes_data]
                ...
                logger.info(f"Validation complete: {len(valid_ids)} Valid, {len(invalid_ids)} Invalid, {total - len(valid_ids) - len(invalid_ids)} Skipped")
