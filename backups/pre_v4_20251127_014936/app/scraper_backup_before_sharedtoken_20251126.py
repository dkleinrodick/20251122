...
class AsyncScraperEngine:
    ...
    async def process_queue(self, tasks: List[Dict[str, str]], mode="auto", ignore_jitter=False, stop_check=None) -> Dict:
        ...
        # Create client pool (lazy auth - authenticate on first use for speed)
        # Match pool size to worker count for best performance (no lock contention)
        ...
        logger.info(f"Creating pool of {pool_size} clients (lazy auth)...")

        for i in range(pool_size):
            proxy = await self.proxy_mgr.get_next_proxy()
            client = FrontierClient(proxy=proxy, timeout=timeout, user_agent=user_agent)
            client_pool.append(client)

        logger.info(f"Client pool ready with {len(client_pool)} clients (will auth on first use)")
        ...
        for i in range(min(worker_count, len(tasks))):
            workers.append(asyncio.create_task(
                self._worker(f"Worker-{i}", queue, results, mode, ignore_jitter, stop_check, client_pool)
            ))
    ...
    async def _worker(self, name: str, queue: asyncio.Queue, results: Dict, mode: str, ignore_jitter: bool, stop_check=None, client_pool=None):
        while True:
            ...
            try:
                # Minimal jitter (reduced from 0.1-5s to 0-0.5s)
                ...
                try:
                    ...
                    async with SessionLocal() as db:
                        ...
                        # 2. Save to FareSnapshot (History) - Only if mode is '3week'
                        if mode == '3week':
