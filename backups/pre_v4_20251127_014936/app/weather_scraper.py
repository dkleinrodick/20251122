...
class WeatherScraper:
    ...
    async def update_forecasts(self, stop_check=None):
        ...
        for i in range(0, total, chunk_size):
            ...
            async with SessionLocal() as session:
                # Collect all airport codes in this chunk for bulk fetch
                chunk_codes = [ap['code'] for ap in chunk]

                # Fetch ALL existing weather data for these airports in ONE query
                stmt = select(WeatherData).where(WeatherData.airport_code.in_(chunk_codes))
                existing_weather = (await session.execute(stmt)).scalars().all()

                # Build lookup dict for O(1) access
                weather_lookup = {}
                for w in existing_weather:
                    key = (w.airport_code, w.date)
                    weather_lookup[key] = w

                # Now process results using the in-memory lookup
                for res_list in results:
                    if not res_list: continue
                    airports_updated += 1
                    for w_data in res_list:
                        key = (w_data['code'], w_data['date'])
                        existing = weather_lookup.get(key)
