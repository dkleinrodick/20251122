from sqlalchemy import Column, Integer, String, Boolean, DateTime, Text, JSON, Date, Float, ARRAY
...
class HeartbeatLog(Base):
    ...
    scrapers_triggered = Column(ARRAY(String), nullable=True) # List of scraper names triggered
    duration_ms = Column(Float, nullable=True)

class ScraperRun(Base):
    __tablename__ = "scraper_runs"
    id = Column(Integer, primary_key=True, index=True)
    job_type = Column(String, index=True) # AutoScraper, MidnightScraper, ThreeWeekScraper, RouteScraper, WeatherScraper
    heartbeat_id = Column(Integer, nullable=True, index=True) # Link to HeartbeatLog
    mode = Column(String, nullable=True) # auto, midnight, 3week, manual, scheduled
    ...
    details = Column(JSON, nullable=True) # Route-level details