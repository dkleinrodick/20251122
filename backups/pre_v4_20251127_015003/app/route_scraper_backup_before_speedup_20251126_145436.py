                ...
                for attempt in range(3):
                    ...
                    async with sem:
                        ...
                        try:
                                    ...
                                    if http_err.response.status_code == 400:
                                        ...
                                        try:
                                            ...
                                            if "does not exist" in msg:
                                                # "The market X - Y does not exist" - mark dead immediately
                                                logger.info(f"[INVALID MARKET] {origin}-{dest} - Market does not exist, stopping checks.")
                                                result["status"] = "invalid"
                                                market_does_not_exist = True
                                                break
                                        except:
                                            pass

                                        # Only log warning if we didn't detect "does not exist"
                                        if not market_does_not_exist:
                            ...
                            if result["status"] == "invalid" and not market_does_not_exist:
                                logger.warning(f"[INVALID] {origin}-{dest} (Checked {days_checked} days, no flights found)")
