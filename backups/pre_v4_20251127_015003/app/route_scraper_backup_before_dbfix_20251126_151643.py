...
class RouteScraper:
    ...
    async def validate_routes(self, stop_check=None):
        ...
        async with SessionLocal() as session:
            ...
            # Create a pool of authenticated clients (reuse to avoid re-auth overhead)
            client_pool = []
            pool_size = min(10, max_concurrent)  # Create 10 clients to share
            logger.info(f"Creating pool of {pool_size} authenticated clients...")
            ...
            if not client_pool:
                logger.error("Failed to create any authenticated clients!")
                return 0

            logger.info(f"Client pool ready with {len(client_pool)} clients")
            ...
            async def check_route(route_dict):
                ...
                # Update DB immediately after checking this route
                async with SessionLocal() as db_session:
                    try:
                        if result["status"] == "valid":
                            await db_session.execute(
                                update(RoutePair)
                                .where(RoutePair.id == route_id)
                        ...
                        logger.error(f"DB update failed for {origin}-{dest}: {e}")

                return result

            # Process all routes at once - no batching, workers continuously process
            logger.info(f"Processing all {total} routes...")
            ...
            try:
                ...
                logger.info(f"Validation complete: {valid_count} Valid, {invalid_count} Invalid, {skip_count} Skipped")
